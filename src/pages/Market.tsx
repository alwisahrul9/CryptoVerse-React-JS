...
export default function Market() {
  ...
  useDocumentTitle("Harga Crypto Hari Ini - Pantau Pergerakan Pasar Crypto Global");

  const [searchQuery, setSearchQuery] = useState<string>("");

  const filteredCoins: CoinTicker[] = data.filter(
    (coin: any) =>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Berdasarkan kapitalisasi pasar global.
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            ...
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800">
                ...
                <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-right flex justify-end items-center gap-1">
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-4 px-6 text-center space-x-2">
                        ...
                        <button
                          onClick={() => {
                            const isInWatchlist = getWatchlistFromStore.some(
                              (item: CoinTicker) => item.id === coin.id,
                            );
                          ...
                          className="p-2 rounded-lg text-slate-400 dark:text-slate-500 hover:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-400/10 transition-all focus:outline-none cursor-pointer"
                        >
                          <Star
                            size={18}
                            fill={
                              getWatchlistFromStore.some(
                                (item: CoinTicker) => item.id === coin.id,
                              )
                                ? "orange"
                                : ""
